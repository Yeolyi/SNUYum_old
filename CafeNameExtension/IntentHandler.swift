...
class IntentHandler: INExtension, ConfigurationIntentHandling {
    ...
    func resolveCafeName(
        for intent: ConfigurationIntent,
        with completion: @escaping (WidgetCafeNameResolutionResult) -> Void) {
        
    }
    
    func provideCafeNameOptionsCollection(
        for intent: ConfigurationIntent,
        with completion: @escaping (INObjectCollection<WidgetCafeName>?, Error?) -> Void) {
